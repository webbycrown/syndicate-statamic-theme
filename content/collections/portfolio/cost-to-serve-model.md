---
id: p3000009-4f56-4789-8abc-000000000009
blueprint: portfolio
title: 'Cost-to-Serve Model'
category: marketing
image:
  - images/projects-gallery-img10.jpg
link_text: 'Consulting'
description: 'Which customers make money, which ones only look busy.'
page_builder:
  -
    id: p30i0009a
    section_subtitle: Portfolio
    section_title: 'Cost-to-Serve Model'
    description: 'Which customers make money, which ones only look busy.'
    type: intro_section
    enabled: true
  -
    id: p30i0009b
    sub_title: 'Cost-to-Serve Model'
    content: 'We worked with Fieldstone Energy to define the constraint, write a short plan, and put names on the work. Which customers make money, which ones only look busy. The engagement ran on a weekly cadence so the board could see movement, not a slide.'
    details_list:
      -
        id: p30d0009a
        key: Category
        value: Consulting
      -
        id: p30d0009b
        key: Client
        value: 'Fieldstone Energy'
      -
        id: p30d0009c
        key: Date
        value: 10/10/2025
      -
        id: p30d0009d
        key: Website
        value: www.fieldstone.energy
    gallery_images:
      - images/projects-gallery-img10.jpg
      - images/projects-gallery-img1.jpg
      - images/projects-gallery-img5.jpg
    full_description: 'Syndicate sat with the Fieldstone Energy leadership team and mapped the current state in one week. Which customers make money, which ones only look busy. We then set a 90-day plan with owners, dates, and a short KPI list. The work stayed practical: a pack the board would read, a process someone could run on leave, and a number that moved. That is how the project became an operating habit instead of a report.'
    accordions:
      -
        id: p30q0009a
        column: left
        question: 'What was the brief?'
        answer: 'Which customers make money, which ones only look busy.'
      -
        id: p30q0009b
        column: left
        question: 'How long did the work take?'
        answer: 'Most programmes run 8 to 12 weeks with a weekly review. The first output is a written current state, not a slide deck.'
      -
        id: p30q0009c
        column: right
        question: 'Who owns the result?'
        answer: 'A named client owner and a Syndicate lead share the cadence. Work that has no owner does not start.'
      -
        id: p30q0009d
        column: right
        question: 'What does success look like?'
        answer: 'Which customers make money, which ones only look busy. The measure is written in week one.'
    type: portfolio_general_content_section
    enabled: true
  -
    id: p30i0009c
    sub_title: 'Related projects'
    portfolios:
      - 68e19570-0cd1-493f-98d4-202a1dc5bf19
      - 5b47539f-c064-4a24-ab74-effdd3e187f8
      - 0962e818-8417-4f4d-ba10-4c79cec17e5f
      - 4fc5cce0-902d-482b-835f-10bc667b00ac
    type: related_portfolio_section
    enabled: true
updated_by: 918c9014-13db-4bd0-be5a-2303bf77da4d
updated_at: 1750600009
seo_title: 'Cost-to-Serve Model'
seo_description: 'Which customers make money, which ones only look busy.'
seo_image: images/projects-gallery-img10.jpg
---
